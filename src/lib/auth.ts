...
export async function updatePreferences(preferences: UserPreferences): Promise<User> {
  const { data } = await api.put<{ user: User }>('/api/auth/me/preferences', { preferences })
  return data.user
}