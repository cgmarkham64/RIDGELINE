...
import { parseDateMs } from './criticalDates.helpers'
      ...
      {formOpen && (
        <div className="pt-2.5 flex flex-col gap-2">
            ...
            {TONE_OPTS.map(t => (
              <button
                ...
                className={`flex-1 px-2 py-1 rounded border font-mono text-label font-bold tracking-[0.08em] uppercase transition-colors cursor-pointer ${
                  draftTone === t.value
                    ? t.activeCls
                    : 'bg-transparent border-border text-text-dim hover:border-border-mid'
                }`}
              >
                {t.label}
              </button>
          ...
          <div className="flex items-center gap-2.5 pt-0.5">
            <button
              onClick={handleAdd}
              disabled={!draftDate.trim() || !draftLabel.trim()}
