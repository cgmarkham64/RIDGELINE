...
const WAYPOINT_SVG_INNER: Record<WaypointType, (c: string) => string> = {
  campsite: (c) => `
  ...
  wildlife: (c) => `
    <ellipse cx="8" cy="11" rx="3.2" ry="2.4" fill="${c}"/>
    <circle cx="4.8" cy="7.8" r="1.5" fill="${c}"/>
    <circle cx="8" cy="6.8" r="1.5" fill="${c}"/>
    <circle cx="11.2" cy="7.8" r="1.5" fill="${c}"/>`,
  viewpoint: (c) => `
...
function waypointSvgString(type: WaypointType, size: number): string {
  ...
  return `<svg width="${size}" height="${size}" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">${WAYPOINT_SVG_INNER[type](c)}</svg>`
}
