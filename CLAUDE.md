...
13. Fix bracket-notation Tailwind classes across all components — the Tailwind v4 migration used `gap-[10px]`, `duration-[80ms]` etc. in many files where shorthand equivalents exist (e.g. `gap-2.5`, `duration-80`). Run a lint pass to catch them all.
14. Fix `FormEvent` deprecation in `AccountDialog.tsx` — React 19 deprecated the re-export of `FormEvent` from `react` (TS6385 warnings on lines 119 and 138). Import from `react` using `React.FormEvent` or switch to the native `SubmitEvent` type.
